...
function isBracketsBalanced(str) {
  ...
  for (const el of str) {
    if (config.indexOf(el) % 2 === 0) {
      stack.push(el);
    } else if (stack[stack.length - 1] === config[config.indexOf(el) - 1]) {
      stack.pop();
    }
  }
...
function toNaryString(num, n) {
  ...
  while (num > 0) {
    ...
    num = parseInt(num / target.length, 10);
  }
  return result;
}
...
function getCommonDirectoryPath(pathes) {
  const path = pathes.map((i) => i.split('/'));
  const rotate = path[0].map((e, i) => path.map((item) => item[i]));
  const equalElements = rotate.filter((arr) => arr.every((e) => e === arr[0]));
